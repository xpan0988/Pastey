...
Profile B uses the production Bridge Device Check with `--profile b`. The harness is only a physical launcher plus read-only verifier: it obtains the generated attempt id from the production self-check report and does not compose a second Profile-B Plan or approval.
...
The Check prepares `Search @ requester → Transfer → Execute @ remote`, but it remains fail-closed until the existing Windows Host has a real selected provider generation, successful health check, selected Host-local managed runtime, verified production ExecutionWorld availability, and the native Windows Stage 1–5 baseline. Production readiness turns a valid selected runtime into the exact Execute process binding; neither the product report nor the harness reads credentials. A missing provider, runtime, or world is `BLOCKED` before managed work starts, not a reason to weaken the gate. Do not claim Profile B PASS until that setup and a two-machine packaged Host run have actually occurred.
...
The provider portion of the product-surface gate is now implemented: each local Host can configure, explicitly select, inspect, update, delete, and health-check its existing provider generation without exposing its credential. The 2.0 UI can operate the authoritative lifecycle for an existing revision, but Draft origination and the detailed topology/result projections required for this smoke are still not renderer-exposed. The A→B→C physical managed smoke is therefore **not yet runnable as a normal product flow**; no physical PASS follows from local/provider tests.
