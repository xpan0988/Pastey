...
The test-only `native_agent::tests::pair_harness` scenarios use separate durable AppPaths, SQLite databases, and Native Agent services for requester and executor. They drive the production movement, regular-file-set package/materialization, native task, reconciliation, Return, apply, cancellation, and Burn paths with a deterministic fake app-server. Run them with `cargo test --manifest-path src-tauri/Cargo.toml --bin pastey native_agent::tests::pair_harness`. The external turn ledger and state dump make duplicate execution and failed transitions diagnosable. These scenarios provide deterministic source-level evidence for authority, movement, persistence, restart, exact Return and conflict recovery, apply, cancel, and Burn. They do not prove real APFS ↔ NTFS behavior, macOS or Windows process behavior, installed Codex app-server semantics or auth/session behavior, sleep/wake, LAN interruption/reconnect, firewall/socket behavior, or packaged release UX. Those boundaries still require physical Mac ↔ Windows acceptance.
