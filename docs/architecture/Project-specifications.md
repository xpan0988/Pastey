...
| Layer 5 - Agent-assisted device workspace | 68% | 88% | Ask Bridge natural-v1 closure for Search and Search -> Return; Transform preview is unsupported/future | High | Manual/two-device validation, transfer-completion smoke, global Activity detail drawer, bounded transform runtime, broad capability coverage, multi-step orchestration, local LLM scheduling, MCP/tool integration, durable peer-identity dependency if needed |
