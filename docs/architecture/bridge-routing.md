...
They must not use broadcast by default. Existing fixed Agent Bridge capability semantics remain unchanged: model output is advisory, host validation and PolicyGate are required, execution requests are host-built, and capability execution requires explicit per-request consent.
