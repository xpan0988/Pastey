# Binary-v2 Early Implementation: Phase 2-4 Scheduler and Runtime Window Foundation

> Status: Completed historical implementation record.
>
> This document describes the early scheduler and runtime-window foundation that prepared Pastey's transfer stack for later binary-v2 or stream-oriented designs. The Phase 2-4 work described here has been completed and is no longer the active roadmap.
>
...
PASTEY_PROFILE=sender PASTEY_APP_DATA_DIR=/tmp/pastey-sender cargo run --profile dev-fast --no-default-features --color always --
```
