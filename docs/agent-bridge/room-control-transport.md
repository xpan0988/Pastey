...
Pastey 1.9.1 product paths use this same selected-peer transport for both Transform + Return and Search + Return. Request file search and candidate payload requests use the canonical room-control selected peer ref for the embedded request target and preview envelope target. Denial status events are terminal product lifecycle events, not failures that retry or continue to execution. Bridge detail uses the same serialized inbox-and-queue pump for automatic polling, focus refresh, and the manual `Check for updates` fallback. Polling continues through sent/delivered and approval/execution states until a product-terminal result, and incoming receiver reviews can open their product panel. Queue refs are updated before pumping so repeated inbox reads do not resend an already delivered event.
