...
Status: Stages 1–3 are implemented: contracts, durable trusted environmental facts, and Core review/root/sealed grant-basis construction. Core can construct bounded process-local task authority through internal typed entry points. Stage 4 has not started. No control session, admitted action, native dispatch or physical execution path exists. Session-bound grants, protocol, execution and native-controller changes below remain proposed.
...
This sequence implements the accepted target in dependency order. Stages 1 and 2 are implemented with focused semantic and durable-ledger tests. Tests in stages 3–9 remain future acceptance requirements.
...
Trusted enrollment, handshake/supervisor facts and qualification evaluator evidence are sealed, non-deserializable inputs. **Only explicit test fakes construct these inputs in Stage 2.** There is no production enrollment/evaluator ingress, authenticated native handshake, supervisor integration, credential/socket handling or native I/O yet. A future producer inside the binding trust boundary must actually authenticate the enrolled native identity and qualify the handshake, or verify the Gate A supervisor's single-writer/incarnation evidence; telemetry and hashes cannot substitute. Gate A provenance is simulation-only and cannot establish a Gate B qualification. This deliberate closed production ingress avoids pretending native identity or qualification has been measured.
