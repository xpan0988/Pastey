...
Disconnect, explicit departure, Burn, and startup recovery invalidate current-session work as applicable; recovery creates no authority. Disconnect retains membership for a possible fresh-session reconnect. Explicit authenticated departure removes the departing peer from current membership but does not delete the survivor's Bridge. Local Burn first cuts local authority off, including Native Agent task/movement authority correlated to that Bridge, then best-effort uses existing authenticated departure/cancellation paths and removes that Host's routes, active server/transfer state, room-control inbox/replay/rate state, candidate/Plan workflow state, Bridge-bound Native Agent envelopes and app-owned retained results, Bridge membership, items, and reusable room material. An authenticated departure applies the same Native Agent authority cutoff for that Bridge on its receiver. Direct local Native Agent tasks without Bridge correlation and user-owned original workspaces are preserved. An unprovable Native Agent apply journal leaves the canonical workspace and uncertain siblings untouched while Bridge authority and its envelope are removed; unknown siblings may remain as orphan disk residue, and burned-room startup still proceeds. Bridge payload items use a per-Bridge wrapping key under the existing installation secret; Burn removes that key before fallible content/database cleanup, so retained Pastey key state cannot decrypt leftover Bridge ciphertext. It preserves user-saved final Inbox files and an independent paired-device display identity only. The durable Burn tombstone is opaque and non-sensitive; it is not product history. Replay, delivery receipts, control inbox entries, logs, and approval records cannot cross a Bridge session as authority. Burn does not prove remote native-process termination across a partition.
