...
The generic peer capability projection carries `0..N` bounded capability facts. An empty projection is a valid observation meaning that the Host currently advertises no concrete bounded capabilities. It is not a malformed response, offline signal, fallback instruction, or authority; Room Control transports and stores it without fabricating a capability.
