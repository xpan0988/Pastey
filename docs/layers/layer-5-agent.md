...
Windows uses a dedicated local `PasteySandboxOffline` principal, not AppContainer. One explicit elevated setup resolves that user from the local Windows account database and requires its exact managed account flags, zero operator authority, and no direct or indirect local-group memberships before reconciling deny-logon rights, a principal-scoped outbound-block Windows Firewall rule, Host-private protected setup state, and Pastey-owned roots under ProgramData. Pastey neither requires nor adds `BUILTIN\Users`; credential proof verifies the exact token SID and rejects privileged well-known group SIDs. The final setup credential is random, DPAPI machine-protected, ACL-readable only by the exact Host user, SYSTEM, and Administrators, and never enters a Worker environment. Before account mutation, setup atomically records Host-user DPAPI-bound provisional state; resume requires its credential proof and exact SID binding. Setup is idempotent for its exact marker/account identity, can resume authenticated Pastey-created partial state, and fails closed on unrelated accounts or substituted/stale identities. A narrowly fingerprinted legacy path recovers only the pre-marker account shape created by the earlier setup implementation.

For each run, the Host stages only exact leases into a Pastey-managed `input/`, `scratch/`, `output/`, and `tools/` layout. The trusted bootstrap receives a Windows-created environment that is not authority-bearing, enters the Host-owned non-breakaway kill-on-close Job before its first instruction, and proves a fresh logon SID over a PID-checked private control pipe. The Host then grants that exact SID read-only input, read/write scratch and output, and read/execute tools; the persistent account receives only parent traversal. Worker-created objects inherit Owner Rights restrictions so the account cannot rewrite their ACLs for a later logon. The bootstrap creates the exact Worker with a `WRITE_RESTRICTED`, maximum-privilege-disabled token, a private desktop, an explicit stdio-only handle list, per-run logon-SID pipes, and only the Host-synthesized bounded environment carried in `RunnerLaunchV1`; the bootstrap environment is never copied, merged, or forwarded. No language/tool enum or command allowlist exists: executable authority remains the exact Host-owned staged resource binding.
