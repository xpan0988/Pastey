...
The separate bounded Host capability probe is diagnostic-only. It uses the fixed `RUNTIME_PROBES` vocabulary through the current authenticated Room Control capability channel and can report a present or unavailable runtime observation for the exact current Host session. It never supplies, discovers for execution, pins, or binds an executable identity. Managed execution continues exclusively through `ManagedRuntimeConfigServiceV1` selection, exact identity validation, existing readiness/admission, `ExecutionWorld`, and Core completion.
