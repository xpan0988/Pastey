...
Pastey persists only the outer Native Agent envelope: immutable task/movement correlation, Host-private Bridge correlation, outer phase/status, exact returned-result snapshot/digest, and source-side apply fact. Restart converts unproved Bridge-bound native work to interrupted/reconciliation-required; it does not recreate a native session or authorize another turn. A durably Completed received task whose snapshot was not sealed before restart can seal the exact result from its retained app-owned task workspace and retry Return without rerunning the Agent. Unsafe or failed capture leaves Agent execution `Completed`; the executor records a consequence failure and the requester learns `Interrupted / native_agent_result_snapshot_recovery_failed` through authenticated reconciliation. The user may explicitly abandon that movement to release source ownership without rerunning the Agent. An unbound local task that cannot be reconciled becomes terminal with `native_agent_interrupted_on_restart` and its unreachable envelope is removed. Reopening a Bridge projects only unresolved safe lifecycle fields into the existing Native Agent card. Remote execution uncertainty (`native_agent_reconciliation_required` or `native_agent_outcome_unknown`) can be reconciled over a freshly resolved Layer 4 binding or explicitly stopped. A fresh reconciliation may also prove a completed executor task and its validated durable snapshot digest after requester session loss. The requester then restores Return authority and retains the expected digest separately from its still-absent result identity; an exact Return can retry without another Agent turn. A snapshotted return can retry only the existing encrypted Transfer. A retained `conflict_recovery_required` result can be revealed or discarded through the existing Host-native actions. An interrupted source apply offers exact Return retry when the durable identity and apply journal agree; the requester checks the same Bridge, task, movement, target Host, and result identity before asking the executing Host to resend its captured snapshot. The executing Host validates that request against its current authenticated session and retained snapshot. `WorkspaceApplyTransactionV1` records the result identity before filesystem changes so restart can restore the original workspace, complete an exact staged result, or recognize a result already installed. A repeated exact Return is acknowledged without reapplying over later user edits. The projection excludes physical paths, native session/turn/process details, provider/auth state, result text, and reasoning/tool history. One canonical workspace remains owned while a Native Agent execution, active movement, reconciliation-required or outcome-unknown fact, or result-apply recovery can still affect it. Explicit cancellation or terminal resolution releases that ownership; ordinary safe interruption may release it. A durable completed apply is idempotent.
