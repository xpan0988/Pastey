...
Windows uses a dedicated local `PasteySandboxOffline` principal, not AppContainer. One explicit elevated setup reconciles that standard offline account, deny-logon rights, a principal-scoped outbound-block Windows Firewall rule, Host-private protected setup state, and Pastey-owned roots under ProgramData. The setup credential is random, DPAPI machine-protected, ACL-readable only by the exact Host user, SYSTEM, and Administrators, and never enters a Worker environment. Setup is idempotent for its exact marker/account identity and fails closed on substituted or partial state.
