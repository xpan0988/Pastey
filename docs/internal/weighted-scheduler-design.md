...
7. Current multi-active assumptions to keep watching include queue UI language that displays one "Current" item, batch-level cancellation expectations, and the legacy progress-correlation fallback for progress events without queue item metadata.
