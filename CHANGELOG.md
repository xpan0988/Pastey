...
Version 1.9.2 is the previous frozen Layer 1–5 baseline. The current `Unreleased` section covers the 1.9.3 managed authority/Worker/native-v2 foundation, production Managed E2E path, Bridge-native capability projection and probes, and low-friction Capability Acquisition confirmation foundation. Version 2.0.0 remains reserved for the complete managed Agent product milestone.
...
- Added Bridge Device Check as the production `Search @ requester → authored Transfer → Execute @ exact remote Host` Managed E2E path, including bounded independent reports and verified fail-closed `BLOCKED / preflight_unavailable` behavior. Real external-provider packaged Mac↔Windows PASS evidence remains pending.
...
- Added the low-friction Capability Acquisition confirmation foundation: exact `HostRef`, generic bounded semantic IDs, renderer-safe display facts, and only `confirmed` / `cancelled`. Actual acquisition behavior is intentionally deferred until AI integration; confirmation performs no installation, probe, process binding, execution authority, Plan/topology mutation, Host selection, or Developer Mode change.
...
- The Worker Context Contract, general cross-representation Transform E2E, real provider conformance, complete Product/Figma wiring, richer coordination recovery, physical failure matrix, Worker network tools, subagents, and Headless Host remain incomplete. Linux managed Process remains unavailable; Windows local Managed Execute is accepted, while real external-provider packaged Mac↔Windows Managed E2E PASS is still pending.
