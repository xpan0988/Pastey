...
Version 1.9.2 is the previous frozen Layer 1–5 baseline. The current `Unreleased` section records both the retained Generic Managed Worker / Execute foundations and the Native mature-Agent architecture migration. Phase 1 — Native Agent Core is closed; Phase 2 — Multi-Device Reliability Closure is current.
...
- Phase 2 Native Agent work remains: physical multi-device movement validation, disconnect/reconnect and stale/replaced-session closure, interrupted-movement and restart recovery, durable conflict-recovery UX, cancellation-race and replay coverage, and the supporting failure matrix/security freeze.
