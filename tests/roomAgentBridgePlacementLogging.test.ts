...
test("Layer 5 docs describe narrow product closure without full-agent overclaim", () => {
  const project = readFileSync("docs/architecture.md", "utf8");
  const safety = readFileSync("docs/layers/layer-5-agent.md", "utf8");
  const contracts = readFileSync("docs/reference.md", "utf8");
  const releaseWorkflow = readFileSync("docs/development.md", "utf8");
  const changelog = readFileSync("CHANGELOG.md", "utf8");
  const docs = [project, safety, contracts, releaseWorkflow, changelog].join("\n");

  assert.match(docs, /bounded Transform authority are implemented/);
  assert.match(docs, /Layer 5 narrow product closure/);
  assert.match(docs, /Ask Bridge natural-v1/);
  assert.match(docs, /Search \/ Transform \/ Return/);
  assert.match(docs, /Search -> Return/);
  assert.match(docs, /Other Transform kinds remain `unsupported_future`/);
  ...
  assert.match(docs, /two-device smoke/);
  assert.match(docs, /Real Transform execution is not available/);
  assert.match(docs, /arbitrary shell, process, file, or network execution/);
