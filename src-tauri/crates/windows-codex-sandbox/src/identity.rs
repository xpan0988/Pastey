...
pub fn require_logon_sandbox_creds(
    ...
    if identity.is_none() {
        let reason = setup_reason.as_deref().unwrap_or("sandbox setup required");
        crate::logging::log_note(reason, Some(&sandbox_dir));
        anyhow::bail!(
            "Windows sandbox setup is missing or out of date; run the Host-owned setup command"
        );
    }
