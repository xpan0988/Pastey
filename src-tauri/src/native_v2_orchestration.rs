...
use crate::{
    ...
    host_identity::{
        ...
        PlanParticipantRef, PlanParticipants,
    },
...
/// Transport-neutral authoritative result proposal. Room Control correlation
/// is validated before conversion on remote ingress and is never fabricated
/// for direct local completion.
#[derive(Clone, Debug, Eq, PartialEq, Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct HostStepResultV1 {
    attempt_id: String,
    approval_id: String,
    plan_id: String,
    revision_id: String,
...
/// An exact remote dispatch request captured while Core changes authoritative
/// state.  It deliberately carries no materialized Room Control event: context
/// resolution may synchronize legacy peer data through a separate SQLite
/// writer, so it must happen only after the authoritative transaction commits.
#[derive(Clone)]
pub(crate) struct NativeV2RemoteIntentV1 {
    pub(crate) room_id: String,
    pub(crate) binding: HostSessionBinding,
...
pub(crate) enum NativeV2CoordinatorActionV1 {
    RemoteIntent(NativeV2RemoteIntentV1),
    LocalReadiness {
...
/// Transport-neutral readiness fact consumed by requester Core. Remote wire
/// validation and local runtime validation both feed this same lifecycle path.
#[derive(Clone)]
struct HostReadinessV1 {
    ...
    async fn native_v2_local_readiness(
        ...
        if managed_here {
            ...
            let selection = if native_required {
                match self.worker_provider_configs.selected_for_managed_workers() {
...
/// Revalidate every captured Host freshness before an authority transaction is
/// opened. Remote resolution may synchronize a legacy peer endpoint through a
/// separate SQLite connection, so doing it under the transaction would create
/// a nested writer. The transaction compares its stored JSON with this exact
/// capture before using it, and remote intent materialization validates again
/// after commit.
fn prevalidate_product_host_freshness(
    runtime: &HostRuntime,
    attempt_id: &str,
    now: i64,
) -> AppResult<HashMap<String, HostExecutionFreshness>> {
    let conn = connection(&runtime.paths)?;
    let mut statement = conn.prepare(
        "SELECT participant_ref, session_binding_json
         FROM native_v2_product_hosts WHERE attempt_id = ?1",
    )?;
    let rows = statement
    ...
    let mut validated = HashMap::with_capacity(rows.len());
    for (participant_ref, binding_json) in rows {
        let freshness: HostExecutionFreshness = serde_json::from_str(&binding_json)?;
        let current = crate::host_runtime::current_host_execution_freshness(runtime, &freshness)?;
        freshness.validate_current(&current, now)?;
        if validated.insert(participant_ref, freshness).is_some() {
            return invalid("Native v2 attempt has duplicate Host participants.");
        }
    }
    Ok(validated)
}

fn prevalidated_freshness_for(
    validated: &HashMap<String, HostExecutionFreshness>,
    participant_ref: &str,
    stored_json: &str,
) -> AppResult<HostExecutionFreshness> {
    let stored: HostExecutionFreshness = serde_json::from_str(stored_json)?;
    let captured = validated.get(participant_ref).ok_or_else(|| {
        AppError::InvalidInput("Native v2 Host freshness was not prevalidated.".into())
    })?;
    if captured != &stored {
        return invalid("Native v2 Host freshness changed during authority update.");
    }
    Ok(stored)
}

fn readiness_result_will_open_preparation(
    paths: &AppPaths,
    attempt_id: &str,
    participant_ref: &str,
) -> AppResult<bool> {
    connection(paths)?
        .query_row(
            "SELECT EXISTS(
             SELECT 1 FROM native_v2_product_attempts a
             JOIN native_v2_product_hosts h ON h.attempt_id = a.attempt_id
             WHERE a.attempt_id = ?1 AND a.state = 'checking_readiness'
             AND h.participant_ref = ?2 AND h.readiness_state = 'pending'
         ) AND NOT EXISTS(
             SELECT 1 FROM native_v2_product_hosts
             WHERE attempt_id = ?1 AND participant_ref != ?2
             AND readiness_state != 'ready'
         )",
            params![attempt_id, participant_ref],
            |row| row.get::<_, i64>(0),
        )
        .map(|value| value == 1)
        .map_err(Into::into)
...
fn prepared_result_will_open_commit(
    paths: &AppPaths,
    attempt_id: &str,
    participant_ref: &str,
) -> AppResult<bool> {
    connection(paths)?
        .query_row(
            "SELECT EXISTS(
             SELECT 1 FROM native_v2_product_attempts a
             JOIN native_v2_product_hosts h ON h.attempt_id = a.attempt_id
             WHERE a.attempt_id = ?1 AND a.state = 'preparing'
             AND h.participant_ref = ?2 AND h.admission_state = 'pending'
         ) AND NOT EXISTS(
             SELECT 1 FROM native_v2_product_hosts
             WHERE attempt_id = ?1 AND participant_ref != ?2
             AND admission_state != 'prepared'
         )",
            params![attempt_id, participant_ref],
            |row| row.get::<_, i64>(0),
        )
...
fn terminate_requester_attempt(
    ...
    let validated_freshness = prevalidate_product_host_freshness(runtime, attempt_id, now)?;
    ...
    for (participant_ref, peer_route_ref, binding_json) in rows {
        let freshness =
            prevalidated_freshness_for(&validated_freshness, &participant_ref, &binding_json)?;
        let target = revision
            .participants
            .as_slice()
            .iter()
        ...
        if let Some(peer_route_ref) = peer_route_ref {
            let binding = freshness.as_remote().ok_or_else(|| {
                AppError::InvalidInput("Native v2 remote cancel binding is unavailable.".into())
            })?;
            if binding.peer_route_ref != peer_route_ref {
                return invalid("Native v2 remote cancel route binding is stale.");
            }
            events.push(NativeV2CoordinatorActionV1::RemoteIntent(
                NativeV2RemoteIntentV1 {
                    room_id: revision.bridge_id.clone(),
                    binding: binding.clone(),
                    kind: CANCEL_KIND,
                    payload: serde_json::to_value(cancel)?,
                },
            ));
        } else if let HostExecutionFreshness::Local(freshness) = freshness {
            events.push(NativeV2CoordinatorActionV1::LocalCancel { cancel, freshness });
        } else {
            return invalid("Native v2 local cancel freshness is unavailable.");
        }
    }
    tx.commit()?;
    Ok(events)
...
pub(crate) fn accept_receiver_cancel(
    ...
    let stored_freshness: HostExecutionFreshness = serde_json::from_str(&row.5)?;
...
pub(crate) fn receiver_attempt_binding(
    paths: &AppPaths,
    attempt_id: &str,
) -> AppResult<Option<HostExecutionFreshness>> {
    connection(paths)?
        .query_row(
...
async fn prepare_requester_attempt(
    ...
    for participant in revision.participants.as_slice() {
        let is_local = participant.host_ref == runtime.local_host_ref;
        if is_local
            && !revision
                .steps
                .iter()
        ...
        let freshness = if is_local {
            HostExecutionFreshness::Local(crate::host_runtime::current_local_execution_freshness(
                runtime,
                &revision.bridge_id,
            )?)
        } else {
        ...
        prepared.push((participant.clone(), freshness, review, readiness));
    }
    ...
    for (participant, freshness, review, readiness) in prepared {
        let peer_route_ref = freshness
            .as_remote()
            .map(|binding| binding.peer_route_ref.clone());
        tx.execute(
            ...
            params![
                ...
                peer_route_ref.as_deref(),
                freshness.authority_ref(),
        ...
        match freshness {
            ...
            HostExecutionFreshness::Remote(binding) => {
                outbound.push(NativeV2CoordinatorActionV1::RemoteIntent(
                    NativeV2RemoteIntentV1 {
                        room_id: revision.bridge_id.clone(),
                        binding: binding.clone(),
                        kind: "bridge_plan.v2.review_request",
...
fn materialize_native_v2_remote_event(
    runtime: &HostRuntime,
    intent: &NativeV2RemoteIntentV1,
) -> AppResult<Value> {
    if intent.room_id != intent.binding.bridge_id {
        return invalid("Native v2 remote dispatch Bridge binding is invalid.");
    }
    let now = crate::storage::now_ts();
    let current = crate::host_runtime::current_host_session_binding(
        runtime,
        &intent.binding.bridge_id,
        &intent.binding.peer_route_ref,
    )?;
    intent.binding.validate_current(&current, now)?;
    let context = crate::room_control::room_control_session_context_for_peer(
        runtime,
        &intent.room_id,
        &intent.binding.peer_route_ref,
    )?;
    if context.room_id != intent.binding.bridge_id
        || context.local_session_ref != intent.binding.local_session_ref
        || context.peer_session_ref != intent.binding.peer_session_ref
        || context.peer_route_ref != intent.binding.peer_route_ref
    {
        return invalid("Native v2 remote dispatch session binding is stale.");
    }
    native_v2_control_event(intent.kind, intent.payload.clone(), &context)
}

async fn send_native_v2_intent(
    runtime: Arc<HostRuntime>,
    intent: NativeV2RemoteIntentV1,
) -> AppResult<crate::room_control::RoomControlDeliveryReceipt> {
    let event = materialize_native_v2_remote_event(&runtime, &intent)?;
    let route =
...
async fn execute_authored_transfer(
    ...
    let representation = artifact.identity.representation();
    let package_path = match artifact.identity.regular_file_set() {
        Some(identity) => Some(crate::regular_file_set_transfer::prepare_package(
            &artifact.path,
            &artifact.scope_root,
            identity,
    ...
    let transport_path = package_path.as_deref().unwrap_or(&artifact.path);
    let item_result = crate::storage::create_outgoing_file_item_with_metadata(
        &runtime.paths,
        &master_key,
        &revision.bridge_id,
        transport_path,
        package_path
            .as_ref()
            .map(|_| "managed-transfer".to_string())
            .or_else(|| Some(artifact.display_name.clone())),
        package_path
            .as_ref()
            .map(|_| "application/octet-stream".to_string())
            .or_else(|| Some(artifact.media_type.clone())),
    );
    if item_result.is_err() {
        if let Some(package_path) = &package_path {
            crate::regular_file_set_transfer::cleanup_package(package_path);
        }
    }
...
fn build_host_step_result(
    ...
    let mut result = HostStepResultV1 {
        attempt_id: attempt_id.into(),
        approval_id: approval.approval_id.clone(),
        plan_id: revision.plan_id.clone(),
        revision_id: revision.revision_id.clone(),
    ...
    result.completion_ref = host_step_completion_ref(&result)?;
    validate_host_step_result(revision, approval, attempt_id, &result)?;
    Ok(result)
}

fn host_step_completion_ref(result: &HostStepResultV1) -> AppResult<String> {
    let mut semantic = result.clone();
    semantic.completion_ref.clear();
    let canonical = canonical_json(&serde_json::to_value(semantic)?);
    Ok(format!(
        "native-v2-local-step-completion:v1:{}",
        blake3::hash(format!("native-v2-local-step-completion:v1\0{canonical}").as_bytes())
            .to_hex()
    ))
}

fn remote_step_result(
    result: HostStepResultV1,
    session_pair_ref: &str,
) -> AppResult<NativeV2StepResultV1> {
    let mut wire = NativeV2StepResultV1 {
        protocol_version: PROTOCOL_VERSION.into(),
        ...
        session_pair_ref: session_pair_ref.to_string(),
        completion_ref: String::new(),
        completed_at: result.completed_at,
    };
    wire.completion_ref = step_result_completion_ref(&wire)?;
    Ok(wire)
}

fn host_result_from_remote(result: &NativeV2StepResultV1) -> HostStepResultV1 {
    HostStepResultV1 {
...
pub(crate) async fn submit_coordinated_step_result(
    ...
    let now = crate::storage::now_ts();
    let current = crate::host_runtime::current_host_execution_freshness(&runtime, &captured)?;
    captured.validate_current(&current, now)?;
    if let HostExecutionFreshness::Local(freshness) = captured {
        let actions = accept_requester_step_result_core(
            &runtime,
            result,
            HostExecutionFreshness::Local(freshness),
            None,
    ...
    let captured = captured
        .as_remote()
        .expect("remote freshness matched above");
    let result = remote_step_result(result, &captured.session_pair_ref)?;
    send_native_v2_intent(
        runtime,
        NativeV2RemoteIntentV1 {
            room_id: captured.bridge_id.clone(),
...
pub(crate) async fn submit_coordinated_attempt_failure(
    ...
    if let HostExecutionFreshness::Local(freshness) = captured {
        let actions = accept_local_host_failure(
            &runtime, &revision, &freshness, attempt_id, step_id, code, now,
        )?;
        return dispatch_native_v2_actions(runtime, actions).await;
    }
    ...
    let captured = captured.as_remote().ok_or_else(|| {
        AppError::InvalidInput("Local failure requires direct Core submission.".into())
    })?;
    send_native_v2_intent(
        runtime,
        NativeV2RemoteIntentV1 {
            ...
            binding: captured.clone(),
            kind: STEP_FAILURE_KIND,
            payload: serde_json::to_value(failure)?,
        },
    )
    .await?;
    Ok(())
...
fn accept_local_host_failure(
    ...
    if code.trim().is_empty() || code.len() > 128 {
        return invalid("Native v2 local failure code is invalid.");
    }
    let requester = participant_for_ref(revision, &revision.requester)
        .ok_or_else(|| AppError::InvalidInput("Native v2 requester vanished.".into()))?;
    if requester.host_ref != runtime.local_host_ref
        || freshness.local_runtime_ref.host_ref() != &runtime.local_host_ref
    {
        return invalid("Native v2 local failure Host authority is invalid.");
    }
    let stored_json: String = connection(&runtime.paths)?.query_row(
        "SELECT h.session_binding_json FROM native_v2_product_hosts h
         JOIN native_v2_product_attempts a ON a.attempt_id = h.attempt_id
         WHERE h.attempt_id = ?1 AND h.participant_ref = ?2
         AND a.state IN ('checking_readiness','preparing','running')",
        params![attempt_id, revision.requester.as_str()],
        |row| row.get(0),
    )?;
    let stored: HostExecutionFreshness = serde_json::from_str(&stored_json)?;
    if stored != HostExecutionFreshness::Local(freshness.clone()) {
        return invalid("Native v2 local failure freshness is stale or substituted.");
    }
    if let Some(step_id) = step_id {
        let step = revision
            .steps
            .iter()
            .find(|step| step.id() == step_id)
            .ok_or_else(|| AppError::InvalidInput("Native v2 failure step vanished.".into()))?;
        if !step_runs_on_host(revision, step, &runtime.local_host_ref) {
            return invalid("Native v2 local failure step belongs to another Host.");
        }
    }
    terminate_requester_attempt(runtime, attempt_id, "failed", code, now)
}
...
pub(crate) async fn accept_readiness_request(
    ...
    let outcome = record_receiver_readiness(
        runtime,
        &review.revision,
        request,
    ...
    Ok(NativeV2ReadinessV1 {
        ...
        revision_hash: outcome.revision_hash,
        bridge_id: outcome.bridge_id,
        participant: outcome.participant,
        host_ref: outcome.host_ref,
...
async fn accept_local_readiness_request(
    ...
    if request.protocol_version != PROTOCOL_VERSION
        || request.expires_at <= now
        || request.bridge_id != freshness.bridge_id
        || request.approval_id != review.approval.approval_id
        || request.plan_id != review.revision.plan_id
        || request.revision_id != review.revision.revision_id
        || request.revision_hash != review.revision.revision_hash
        || request.sender != review.sender
        || request.target != review.target
        || request.target != review.revision.requester
        || request.expires_at > review.approval.expires_at
        || freshness.local_runtime_ref.host_ref() != &runtime.local_host_ref
    {
        return invalid("Native v2 local readiness crossed exact Plan or runtime authority.");
    }
    record_receiver_readiness(
        runtime,
        &review.revision,
        request,
...
pub(crate) fn accept_requester_readiness(
    ...
    accept_requester_readiness_core(
        runtime,
        HostReadinessV1 {
...
fn accept_requester_readiness_core(
    runtime: &Arc<HostRuntime>,
    result: HostReadinessV1,
    captured: HostExecutionFreshness,
    now: i64,
) -> AppResult<Vec<NativeV2CoordinatorActionV1>> {
    if result.expires_at <= now || result.bridge_id != captured.bridge_id() {
        return invalid("Native v2 readiness authority is invalid.");
    }
    let validated_freshness = if result.ready
        && readiness_result_will_open_preparation(
            &runtime.paths,
            &result.attempt_id,
            result.participant.as_str(),
        )? {
        Some(prevalidate_product_host_freshness(
            runtime,
            &result.attempt_id,
            now,
        )?)
    ...
    let stored_freshness: HostExecutionFreshness =
    ...
    drop(statement);
    let mut events = Vec::new();
    let validated_freshness = validated_freshness.as_ref().ok_or_else(|| {
        AppError::InvalidInput("Native v2 Host freshness changed before preparation.".into())
    })?;
    for (participant_ref, peer_route_ref, binding_json, review_json) in rows {
        ...
        if let Some(peer_route_ref) = peer_route_ref {
            let binding = freshness.as_remote().ok_or_else(|| {
                AppError::InvalidInput("Native v2 remote start binding is unavailable.".into())
            })?;
            if binding.peer_route_ref != peer_route_ref {
                return invalid("Native v2 remote start route binding is stale.");
            }
            events.push(NativeV2CoordinatorActionV1::RemoteIntent(
                NativeV2RemoteIntentV1 {
                    room_id: revision.bridge_id.clone(),
                    binding: binding.clone(),
                    kind: "bridge_plan.v2.attempt_start",
                    payload: serde_json::to_value(start)?,
                },
            ));
        } else if let HostExecutionFreshness::Local(freshness) = freshness {
            events.push(NativeV2CoordinatorActionV1::LocalAttemptStart { start, freshness });
        } else {
            return invalid("Native v2 local attempt freshness is unavailable.");
        }
    }
    tx.execute(
...
pub(crate) fn record_receiver_prepared(
    ...
    let prepared = record_receiver_prepared_core(
        paths,
        start,
        accepted,
        HostExecutionFreshness::Remote(captured.clone()),
    ...
    Ok(NativeV2PreparedV1 {
        ...
        admission_ref: prepared.admission_ref,
        session_pair_ref: captured.session_pair_ref.clone(),
        expires_at: prepared.expires_at,
    })
}

fn record_local_receiver_prepared(
    paths: &AppPaths,
    start: &AttemptStartV2,
    accepted: &crate::bridge_plan_v2::AcceptedAttemptV2,
    freshness: &LocalExecutionFreshness,
    now: i64,
) -> AppResult<HostPreparedV1> {
    record_receiver_prepared_core(
        paths,
        start,
        accepted,
        HostExecutionFreshness::Local(freshness.clone()),
...
fn record_receiver_prepared_core(
    paths: &AppPaths,
    start: &AttemptStartV2,
    accepted: &crate::bridge_plan_v2::AcceptedAttemptV2,
    captured: HostExecutionFreshness,
    ...
    let reviewed_freshness: HostExecutionFreshness = serde_json::from_str(&review.2)?;
    if reviewed_freshness != captured
...
pub(crate) fn accept_requester_prepared(
    ...
    accept_requester_prepared_core(
        runtime,
        HostPreparedV1 {
            correlation_id: prepared.correlation_id,
            attempt_id: prepared.attempt_id,
...
fn accept_requester_prepared_core(
    runtime: &Arc<HostRuntime>,
    prepared: HostPreparedV1,
    captured: HostExecutionFreshness,
    now: i64,
) -> AppResult<Vec<NativeV2CoordinatorActionV1>> {
    if prepared.expires_at <= now
        ...
        || prepared.admission_ref.trim().is_empty()
    {
        return invalid("Native v2 prepared authority is invalid.");
    }
    let validated_freshness = if prepared_result_will_open_commit(
        &runtime.paths,
        &prepared.attempt_id,
    ...
    let mut events = Vec::new();
    let validated_freshness = validated_freshness.as_ref().ok_or_else(|| {
        AppError::InvalidInput("Native v2 Host freshness changed before attempt commit.".into())
    })?;
    for (participant_ref, peer_route_ref, binding_json) in rows {
        let freshness =
            prevalidated_freshness_for(&validated_freshness, &participant_ref, &binding_json)?;
        ...
        if let Some(peer_route_ref) = peer_route_ref {
            let binding = freshness.as_remote().ok_or_else(|| {
                AppError::InvalidInput("Native v2 remote commit binding is unavailable.".into())
            })?;
            if binding.peer_route_ref != peer_route_ref {
                return invalid("Native v2 remote commit route binding is stale.");
            }
            events.push(NativeV2CoordinatorActionV1::RemoteIntent(
                NativeV2RemoteIntentV1 {
                    room_id: revision.bridge_id.clone(),
                    binding: binding.clone(),
                    kind: COMMIT_KIND,
                    payload: serde_json::to_value(commit)?,
                },
            ));
        } else if let HostExecutionFreshness::Local(freshness) = freshness {
            events.push(NativeV2CoordinatorActionV1::LocalAttemptCommit { commit, freshness });
        } else {
            return invalid("Native v2 local commit freshness is unavailable.");
        }
    }
    tx.execute(
...
pub(crate) fn accept_receiver_commit(
    ...
    let stored_freshness: HostExecutionFreshness = serde_json::from_str(&row.2)?;
...
pub(crate) fn accept_requester_step_result(
    ...
    let host_result = host_result_from_remote(&result);
    accept_requester_step_result_core(
        runtime,
        host_result,
        HostExecutionFreshness::Remote(captured.clone()),
        Some(result),
...
fn accept_requester_step_result_core(
    runtime: &Arc<HostRuntime>,
    result: HostStepResultV1,
    captured: HostExecutionFreshness,
    remote_result: Option<NativeV2StepResultV1>,
    now: i64,
) -> AppResult<Vec<NativeV2CoordinatorActionV1>> {
    if result.bridge_id != captured.bridge_id() {
        return invalid("Native v2 step result runtime authority is invalid.");
    }
    let validated_freshness = prevalidate_product_host_freshness(runtime, &result.attempt_id, now)?;
    let mut conn = connection(&runtime.paths)?;
    let tx = conn.transaction()?;
    let row = tx
        .query_row(
    ...
    validate_host_step_result(&revision, &approval, &result.attempt_id, &result)?;
    let step = revision
        .steps
        .iter()
        ...
        .filter(|participant| step.binds_participant(&participant.participant_ref))
    {
        ...
        let local_freshness = prevalidated_freshness_for(
            &validated_freshness,
            local_participant.participant_ref.as_str(),
            &local_binding_json,
        )?;
        validate_receiver_host_step_commit_prerequisites(
            &tx,
            runtime,
            &revision,
            &approval,
            &result.attempt_id,
            &revision.requester,
            &local_participant.participant_ref,
            &result,
            row.3,
            &local_freshness,
            now,
    ...
    for (participant_ref, peer_route_ref, binding_json) in rows {
        ...
        if let Some(peer_route_ref) = peer_route_ref {
            let binding = freshness.as_remote().ok_or_else(|| {
                AppError::InvalidInput(
                    "Native v2 remote step-commit binding is unavailable.".into(),
                )
            })?;
            if binding.peer_route_ref != peer_route_ref {
                return invalid("Native v2 remote step-commit route binding is stale.");
            }
            let wire_result = match &remote_result {
                Some(result) => result.clone(),
                None => remote_step_result(result.clone(), &binding.session_pair_ref)?,
            };
            ...
            events.push(NativeV2CoordinatorActionV1::RemoteIntent(
                NativeV2RemoteIntentV1 {
                    room_id: revision.bridge_id.clone(),
                    binding: binding.clone(),
                    kind: STEP_COMMIT_KIND,
                    payload: serde_json::to_value(commit)?,
                },
            ));
        } else if let HostExecutionFreshness::Local(freshness) = freshness {
            events.push(NativeV2CoordinatorActionV1::LocalStepCommit {
                attempt_id: result.attempt_id.clone(),
                result: result.clone(),
                freshness,
            });
        } else {
            return invalid("Native v2 local step-commit freshness is unavailable.");
        }
    }
    tx.commit()?;
    Ok(events)
...
fn accept_local_receiver_step_commit(
    runtime: &HostRuntime,
    attempt_id: &str,
    result: &HostStepResultV1,
    captured: &HostExecutionFreshness,
    ...
    let target = revision
        ...
        .iter()
        .find(|participant| participant.host_ref == runtime.local_host_ref)
        .ok_or_else(|| AppError::InvalidInput("Local native v2 participant vanished.".into()))?;
    validate_receiver_host_step_commit_prerequisites(
        &conn,
        runtime,
        &revision,
        ...
        attempt_id,
        &revision.requester,
        &target.participant_ref,
        result,
        approval.expires_at,
        captured,
        now,
    )?;
    record_host_step_commit(
        &runtime.paths,
        &revision,
        &approval,
        attempt_id,
    ...
    let complete = revision
        .steps
        .iter()
        .all(|step| committed_step(&conn, attempt_id, step.id()).unwrap_or(false));
    if complete {
        connection(&runtime.paths)?.execute(
            "UPDATE native_v2_receiver_attempts SET state = 'completed', updated_at = ?2
             WHERE attempt_id = ?1 AND state = 'running'",
...
fn validate_receiver_step_commit_prerequisites(
    ...
    validate_receiver_host_step_commit_prerequisites(
        ...
        &commit.sender,
        &commit.target,
        &host_result_from_remote(&commit.result),
        commit.expires_at,
        captured,
        now,
    )
...
#[allow(clippy::too_many_arguments)]
fn validate_receiver_host_step_commit_prerequisites(
    conn: &Connection,
    runtime: &HostRuntime,
    revision: &PlanRevisionV2,
    approval: &PlanApprovalV2,
    attempt_id: &str,
    sender: &PlanParticipantRef,
    target: &PlanParticipantRef,
    result: &HostStepResultV1,
    expires_at: i64,
    captured: &HostExecutionFreshness,
    ...
    if expires_at <= now || result.bridge_id != captured.bridge_id() {
        return invalid("Native v2 step commit authority is invalid.");
    }
    let row = conn
        .query_row(
            "SELECT requester_participant_ref, target_participant_ref,
    ...
    for dependency in step.dependencies() {
        if !committed_step(conn, attempt_id, dependency)? {
    ...
    } = step
    {
        ...
        if destination_host.host_ref == runtime.local_host_ref
            && !has_exact_transfer_receipt(
                ...
                revision,
                output,
                result.content_digest.as_deref().unwrap_or_default(),
...
fn action_attempt_id(action: Option<&NativeV2CoordinatorActionV1>) -> Option<String> {
    match action? {
        NativeV2CoordinatorActionV1::RemoteIntent(intent) => intent
            .payload
            .get("attemptId")
        ...
        NativeV2CoordinatorActionV1::LocalStepCommit { attempt_id, .. } => Some(attempt_id.clone()),
...
async fn dispatch_native_v2_actions(
    ...
    while let Some(action) = pending.pop_front() {
        let now = crate::storage::now_ts();
        match action {
            NativeV2CoordinatorActionV1::RemoteIntent(intent) => {
                send_native_v2_intent(runtime.clone(), intent).await?;
            }
            NativeV2CoordinatorActionV1::LocalReadiness {
                ...
                let current = crate::host_runtime::current_local_execution_freshness(
                    &runtime,
                    &freshness.bridge_id,
                )?;
                freshness.validate_current(&current, now)?;
            ...
            NativeV2CoordinatorActionV1::LocalAttemptStart { start, freshness } => {
                let current = crate::host_runtime::current_local_execution_freshness(
                    &runtime,
                    &freshness.bridge_id,
                )?;
                freshness.validate_current(&current, now)?;
                let decision = runtime.accept_local_live_v2_managed_attempt_deferred(
                    start.clone(),
                    freshness.clone(),
                    current,
                    now,
                ...
                let crate::bridge_plan_v2::AttemptStartDecisionV2::Accepted(accepted) = decision
                    ...
                    return invalid("Local Host admission denied.");
                };
                let prepared = record_local_receiver_prepared(
                    &runtime.paths,
                    &start,
                    &accepted,
                    &freshness,
                ...
                pending.extend(accept_requester_prepared_core(
                    &runtime,
                    prepared,
                    HostExecutionFreshness::Local(freshness),
                    now,
            ...
            NativeV2CoordinatorActionV1::LocalAttemptCommit { commit, freshness } => {
                let captured = HostExecutionFreshness::Local(freshness);
                let current =
                    crate::host_runtime::current_host_execution_freshness(&runtime, &captured)?;
                captured.validate_current(&current, now)?;
                accept_receiver_commit(&runtime, &commit, &captured, now)?;
                start_receiver_attempt(runtime.clone(), commit.attempt_id, captured);
            }
            NativeV2CoordinatorActionV1::LocalStepCommit {
                attempt_id,
                result,
                freshness,
            } => {
                ...
                let current =
                    crate::host_runtime::current_host_execution_freshness(&runtime, &captured)?;
                captured.validate_current(&current, now)?;
                accept_local_receiver_step_commit(&runtime, &attempt_id, &result, &captured, now)?;
                start_receiver_attempt(runtime.clone(), attempt_id, captured);
            }
            NativeV2CoordinatorActionV1::LocalCancel { cancel, freshness } => {
                let captured = HostExecutionFreshness::Local(freshness);
                let current =
                    crate::host_runtime::current_host_execution_freshness(&runtime, &captured)?;
                captured.validate_current(&current, now)?;
                let transfer_id = match accept_receiver_cancel(&runtime, &cancel, &captured, now) {
                    Ok(value) => value,
...
pub(crate) fn validate_step_result(
    ...
    validate_host_step_result(
        revision,
        approval,
        expected_attempt_id,
        &host_result_from_remote(result),
    )
}

fn validate_host_step_result(
    revision: &PlanRevisionV2,
    approval: &PlanApprovalV2,
    expected_attempt_id: &str,
    result: &HostStepResultV1,
    ...
    if result.attempt_id != expected_attempt_id
        ...
        || result.revision_hash != revision.revision_hash
        || result.bridge_id != revision.bridge_id
        || result.completion_ref.trim().is_empty()
    {
        return invalid("Native v2 step result semantic correlation is invalid.");
    }
    let step = revision
        .steps
        .iter()
...
pub(crate) fn register_transfer_landing(
        ...
        == crate::managed_objects::ManagedArtifactRepresentationV1::RegularFileSet
    {
        ...
        let scope_root = tree
            ...
            .ok_or_else(|| {
                AppError::InvalidInput(
                    "Native v2 regular-file-set transfer root is unavailable.".into(),
                )
            })?;
        let result = runtime.managed_objects.lock().bind_transferred_revision(
            crate::managed_objects::HostArtifactAcquisition {
                ...
                path: tree.clone(),
                scope_root,
                display_name: "managed-input".into(),
        ...
        {
            crate::regular_file_set_transfer::cleanup_materialized_tree(&tree);
            return invalid("Native v2 regular-file-set Transfer receipt content is invalid.");
        }
        if let Err(error) = insert_transfer_receipt(runtime, metadata, &acquisition, now) {
...
fn insert_transfer_receipt(
    runtime: &HostRuntime,
    metadata: &NativeV2TransferMetadataV1,
    acquisition: &crate::managed_objects::ManagedObjectAcquisition,
    now: i64,
) -> AppResult<()> {
    connection(&runtime.paths)?.execute(
...
mod tests {
    ...
    async fn install_remote_room_control_peer(
        ...
        storage::upsert_bridge_peer_endpoint(
            ...
            &StoredBridgePeerEndpoint {
                ...
                logical_host_ref: Some(remote_host.as_str().into()),
                durable_identity_id: None,
                updated_at: now,
            },
        )
        .unwrap();
        // Exercise the legacy synchronization write which originally collided
        // with requester authority transactions on a second SQLite connection.
        connection(&runtime.paths)
            .unwrap()
            .execute(
    ...
    fn remote_search_revision(runtime: &HostRuntime, remote_host: &HostRef) -> PlanRevisionV2 {
        ...
        compose_revision(NativeV2ComposeRequestV1 {
            plan_id: "remote-sqlite-plan".into(),
            revision_id: "remote-sqlite-revision".into(),
            revision_number: 1,
            bridge_id: "bridge-native-v2".into(),
            requester_host_ref: runtime.local_host_ref.as_str().into(),
            participant_host_refs: vec![
                runtime.local_host_ref.as_str().into(),
                remote_host.as_str().into(),
            ],
            roots: Vec::new(),
            original_user_goal: "Search on the exact remote Host.".into(),
            expected_outcome: "Core commits one exact Search result.".into(),
            steps: vec![NativeV2StepDraftV1::Search {
                step_id: "search-remote".into(),
                depends_on: Vec::new(),
                host_ref: remote_host.as_str().into(),
                output: object,
    ...
    fn remote_intent(
        actions: &[NativeV2CoordinatorActionV1],
        kind: &str,
    ) -> NativeV2RemoteIntentV1 {
        actions
            .iter()
            .find_map(|action| match action {
                NativeV2CoordinatorActionV1::RemoteIntent(intent) if intent.kind == kind => {
                    Some(intent.clone())
                }
                _ => None,
            })
            .unwrap_or_else(|| panic!("missing remote intent {kind}"))
    }

    fn seed_remote_search_running(
        runtime: &HostRuntime,
        remote_host: &HostRef,
        remote_binding: &HostSessionBinding,
        now: i64,
    ) -> (PlanRevisionV2, PlanApprovalV2) {
        ...
        let approval = store
            .approve(
                &revision.revision_id,
                "approval-remote-sqlite",
                now + 600,
                now,
            )
            .unwrap();
        let local_freshness =
            crate::host_runtime::current_local_execution_freshness(runtime, &revision.bridge_id)
                .unwrap();
        let conn = connection(&runtime.paths).unwrap();
        conn.execute(
            "INSERT INTO native_v2_product_attempts
             (attempt_id, approval_id, revision_id, revision_hash, state, failure_code,
              expires_at, created_at, updated_at)
             VALUES ('attempt-remote-sqlite', ?1, ?2, ?3, 'running', NULL, ?4, ?5, ?5)",
            params![
                approval.approval_id,
                revision.revision_id,
                revision.revision_hash,
        ...
        conn.execute(
            "INSERT INTO native_v2_product_steps
             (revision_id, attempt_id, step_id, operation, state, completion_ref, updated_at)
             VALUES (?1, 'attempt-remote-sqlite', 'search-remote', 'search', 'eligible', NULL, ?2)",
            params![revision.revision_id, now],
        )
        .unwrap();
        for (participant, host_ref, route, freshness) in [
            (
                PlanParticipantRef::for_host(&revision.plan_id, remote_host).unwrap(),
                remote_host.clone(),
                Some(remote_binding.peer_route_ref.as_str()),
                HostExecutionFreshness::Remote(remote_binding.clone()),
            ),
            (
                PlanParticipantRef::for_host(&revision.plan_id, &runtime.local_host_ref).unwrap(),
                runtime.local_host_ref.clone(),
                None,
                HostExecutionFreshness::Local(local_freshness),
            ),
        ] {
            conn.execute(
                "INSERT INTO native_v2_product_hosts
                 ...
                 VALUES ('attempt-remote-sqlite', ?1, ?2, ?3, ?4, ?5,
                         'ready', 'committed', ?6)",
                params![
                    participant.as_str(),
                    host_ref.as_str(),
    ...
    async fn file_backed_remote_attempt_preparation_commits_before_context_materialization() {
        ...
        store
            .create_draft(&revision, &runtime.local_host_ref, now)
            .unwrap();
        store
            .approve(
                &revision.revision_id,
                "approval-sqlite-prepare",
                now + 600,
                now,
            )
            .unwrap();

        let actions = prepare_requester_attempt(
            &runtime,
            "approval-sqlite-prepare",
        ...
        assert_eq!(
            ...
            Some("bridge_plan.v2.review_request")
        );
        let state: String = connection(&paths)
            .unwrap()
            .query_row(
                "SELECT state FROM native_v2_product_attempts WHERE attempt_id = 'attempt-sqlite-prepare'",
                [],
                |row| row.get(0),
            )
            .unwrap();
    ...
    async fn file_backed_remote_step_commit_materializes_after_authoritative_commit() {
        ...
        let remote_participant =
            PlanParticipantRef::for_host(&revision.plan_id, &remote_host).unwrap();
        let step = revision.steps.first().unwrap();
        let output = match step {
            PlanStepV2::Search { output, .. } => output.clone(),
            _ => panic!("remote SQLite fixture must use Search"),
        };
        let result = build_step_result(
            &revision,
            &approval,
            "attempt-remote-sqlite",
            step,
            remote_participant,
            remote_host,
            Some(output),
            Some("search-content-digest".into()),
            None,
            binding.session_pair_ref.clone(),
            now,
        )
        .unwrap();

        let actions = accept_requester_step_result(&runtime, result, &binding, now).unwrap();
        let intent = remote_intent(&actions, STEP_COMMIT_KIND);
        let event = materialize_native_v2_remote_event(&runtime, &intent).unwrap();
        assert_eq!(
            event.get("kind").and_then(Value::as_str),
            Some(STEP_COMMIT_KIND)
        );
        let commits: i64 = connection(&paths)
            .unwrap()
            .query_row(
                "SELECT COUNT(*) FROM native_v2_step_commits WHERE attempt_id = 'attempt-remote-sqlite'",
                [],
                |row| row.get(0),
            )
            .unwrap();
    ...
    async fn file_backed_terminal_propagation_commits_before_remote_context_materialization() {
        ...
        store
            .approve(
                &revision.revision_id,
                "approval-sqlite-cancel",
                now + 600,
                now,
            )
            .unwrap();
        prepare_requester_attempt(
            &runtime,
            "approval-sqlite-cancel",
            "attempt-sqlite-cancel",
            now + 600,
            now,
        )
        .await
        ...
        .unwrap();
        let intent = remote_intent(&actions, CANCEL_KIND);
        assert!(materialize_native_v2_remote_event(&runtime, &intent).is_ok());
        let state: String = connection(&paths)
            .unwrap()
            .query_row(
                "SELECT state FROM native_v2_product_attempts WHERE attempt_id = 'attempt-sqlite-cancel'",
                [],
                |row| row.get(0),
            )
            .unwrap();
    ...
    async fn remote_intent_fails_closed_when_exact_route_session_is_substituted() {
        ...
        let intent = NativeV2RemoteIntentV1 {
            ...
            binding: binding.clone(),
            kind: READINESS_REQUEST_KIND,
            payload: serde_json::json!({ "attemptId": "attempt-sqlite-substitution" }),
        };
        let replacement_secret = crate::crypto::random_key();
        storage::upsert_bridge_peer_endpoint(
            &paths,
            &StoredBridgePeerEndpoint {
                room_id: binding.bridge_id.clone(),
                peer_session_id: binding.peer_route_ref.clone(),
                display_name: Some("substituted-remote".into()),
                endpoint_host: Some("127.0.0.1".into()),
                endpoint_port: Some(9),
                transport_public_key: Some(crate::crypto::encode_key(
                    &crate::crypto::transport_public_key(&replacement_secret),
                )),
                liveness: BridgePeerLiveness::Connected,
                join_method: BridgePeerJoinMethod::ManualCode,
                logical_host_ref: Some(remote_host.as_str().into()),
        ...
        .unwrap();
        assert!(materialize_native_v2_remote_event(&runtime, &intent).is_err());
    }
    ...
    struct RequesterLocalTransformFixture {
        ...
        runtime: Arc<HostRuntime>,
        revision: PlanRevisionV2,
        binding: HostExecutionFreshness,
        attempt_id: String,
    }
    ...
    async fn requester_local_transform_fixture(
        ...
        let binding = receiver_attempt_binding(&paths, &attempt_id)
            .unwrap()
            .unwrap();
        assert!(matches!(&binding, HostExecutionFreshness::Local(_)));
        RequesterLocalTransformFixture {
            paths,
            runtime,
            revision,
    ...
    async fn requester_local_search_uses_full_admission_lifecycle_and_completes_once() {
        ...
        let binding = receiver_attempt_binding(&paths, "attempt-requester-local-search")
            ...
            .unwrap();
        assert!(matches!(&binding, HostExecutionFreshness::Local(_)));
        let local_freshness_json = serde_json::to_value(&binding).unwrap();
        assert!(local_freshness_json.get("sessionPairRef").is_none());
        assert!(local_freshness_json.get("peerRouteRef").is_none());
        ...
        let receiver_state: String = conn
            ...
            .unwrap();
        let protocol_replay_claims: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM bridge_plan_v2_protocol_messages
                 WHERE bridge_id = 'bridge-native-v2'",
                [],
                |row| row.get(0),
            )
            .unwrap();
        ...
        assert_eq!(protocol_replay_claims, 0);
    ...
    async fn requester_local_restarted_session_cannot_finalize_or_unlock_successor() {
        ...
        let current =
            crate::host_runtime::current_host_execution_freshness(&restarted, &fixture.binding)
                .unwrap();
        assert_ne!(current.authority_ref(), fixture.binding.authority_ref());
        let mut provider = RequesterLocalTransformProvider::exact_revision_two();
        let error = restarted
            .dispatch_next_v2_managed_with_provider(
                &fixture.attempt_id,
    ...
    #[tokio::test]
    async fn cancellation_prevents_late_local_transform_success() {
        let fixture =
            requester_local_transform_fixture("native-v2-local-cancelled-transform", false).await;
        let cancelled = fixture
            .runtime
            .cancel_native_v2_product_attempt(&fixture.attempt_id, storage::now_ts())
            .await
            .unwrap();
        assert_eq!(cancelled.state, NativeV2ProductStateV1::Cancelled);
        ...
        let conn = connection(&fixture.paths).unwrap();
        for table in ["bridge_plan_v2_transform_results", "native_v2_step_commits"] {
            let count: i64 = conn
                .query_row(
                    &format!("SELECT COUNT(*) FROM {table} WHERE attempt_id = ?1"),
                    [&fixture.attempt_id],
                    |row| row.get(0),
                )
                .unwrap();
            assert_eq!(count, 0, "{table} must remain empty after cancellation");
        }
        drop(conn);
        let _ = std::fs::remove_dir_all(fixture.paths.app_data_dir);
    }
    ...
    fn requester_local_transfer_receipt_is_checked_before_shared_commit_visibility() {
        ...
        for (participant, host_ref, route, binding) in [
            ...
            (
                ...
                &requester_host,
                None,
                HostExecutionFreshness::Local(local_binding.clone()),
            ),
        ] {
            conn.execute(
                "INSERT INTO native_v2_product_hosts
        ...
        conn.execute(
            ...
            params![
                ...
                approval.approval_id,
                requester.as_str(),
                local_binding.local_runtime_ref.generation_ref(),
                serde_json::to_string(&HostExecutionFreshness::Local(local_binding.clone()))
                    .unwrap(),
                now + 600,
                now
            ],
    ...
    #[test]
    fn regular_file_set_transfer_receipt_relocates_the_exact_revision_without_lineage() {
        let paths = paths("native-v2-regular-file-set-transfer");
        let runtime = requester_runtime(paths.clone(), "destination-host");
        let source_root = paths.temp_dir.join("source-tree");
        ...
        .unwrap();
        std::fs::write(source_root.join("src/main.rs"), b"fn main() {}\n").unwrap();
        std::fs::write(
        ...
        .unwrap();
        std::fs::write(
            source_root.join("tests/integration.rs"),
            b"#[test] fn it_works() {}\n",
        )
        .unwrap();
        ...
        .unwrap();
        let plan_id = "file-set-transfer-plan";
        let source_host = host("source-host");
        let source = PlanParticipantRef::for_host(plan_id, &source_host).unwrap();
        let destination = PlanParticipantRef::for_host(plan_id, &runtime.local_host_ref).unwrap();
        let object = ManagedObjectRevisionV2 {
            logical_object_id: format!("managed-object:v1:{}", "d".repeat(64)),
            revision: 7,
        };
        let metadata = NativeV2TransferMetadataV1 {
            protocol_version: PROTOCOL_VERSION.into(),
            attempt_id: "attempt-file-set-transfer".into(),
            approval_id: "approval-file-set-transfer".into(),
            plan_id: plan_id.into(),
            revision_id: "revision-file-set-transfer".into(),
            revision_hash: "hash-file-set-transfer".into(),
            bridge_id: "bridge-native-v2".into(),
            step_id: "transfer-file-set".into(),
            source,
            destination,
            source_host_ref: source_host,
        ...
        let acquired = runtime
            ...
            .unwrap();
        assert_eq!(acquired.object.revision, 7);
        assert_eq!(acquired.object.host_ref, runtime.local_host_ref);
        assert_eq!(
        ...
        let artifact = runtime
            ...
            .unwrap();
        let received = artifact.identity.regular_file_set().unwrap();
        ...
        assert_eq!(
            std::fs::read(artifact.path.join("src/lib.rs")).unwrap(),
            b"pub fn value() -> u8 { 7 }\n"
        );
        let receipts: i64 = connection(&paths)
            .unwrap()
            .query_row(
                "SELECT COUNT(*) FROM native_v2_transfer_receipts
                 WHERE attempt_id = 'attempt-file-set-transfer' AND step_id = 'transfer-file-set'",
                [],
                |row| row.get(0),
            )
            .unwrap();
        ...
        assert!(!artifact
            .path
            .starts_with(paths.temp_dir.join("native-v2-transfers")));
        let _ = std::fs::remove_dir_all(paths.app_data_dir);
    }

    #[test]
    fn transfer_metadata_without_file_set_fields_remains_a_scalar_transfer() {
        let source_host = host("scalar-source");
        let destination_host = host("scalar-destination");
        let plan_id = "scalar-transfer-plan";
        let metadata = NativeV2TransferMetadataV1 {
            protocol_version: PROTOCOL_VERSION.into(),
            attempt_id: "attempt-scalar-transfer".into(),
            approval_id: "approval-scalar-transfer".into(),
            plan_id: plan_id.into(),
            revision_id: "revision-scalar-transfer".into(),
            revision_hash: "hash-scalar-transfer".into(),
            bridge_id: "bridge-native-v2".into(),
            step_id: "transfer-scalar".into(),
            source: PlanParticipantRef::for_host(plan_id, &source_host).unwrap(),
            destination: PlanParticipantRef::for_host(plan_id, &destination_host).unwrap(),
            source_host_ref: source_host,
            destination_host_ref: destination_host,
            object: ManagedObjectRevisionV2 {
                logical_object_id: format!("managed-object:v1:{}", "e".repeat(64)),
