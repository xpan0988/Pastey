...
fn write_report(
    ...
    let report = json!({
        "schemaVersion": "pastey-native-v2-physical-evidence-v1",
        "profile": profile.label(), "role": role,
        "machine": {"os": std::env::consts::OS, "arch": std::env::consts::ARCH, "gitCommit": git_commit(), "harnessVersion": env!("CARGO_PKG_VERSION"), "product": product_identity(product)},
    ...
    fs::write(text_path, format!("Native-v2 physical evidence\nprofile: {}\nrole: {}\nattempt: {}\nrevision: {}\nrequester state: {}\ncommits: {}\nreceipts: {}\nSQLite integrity: {}\nJSON: {}\n", profile.label(), role, attempt_id, report["attempt"]["revisionId"], report["attempt"]["finalPlanState"], report["counts"]["stepCommitCount"], report["counts"]["receiptCount"], report["counts"]["integrity"], json_path.display()))?;
...
fn verify(profile: Profile, args: &BTreeMap<String, String>) -> AppResult<()> {
    ...
    {
        failures.push("requester Core does not show every participant ready and committed");
    }
    ...
    {
        failures.push("Windows receipt does not bind this exact revision and destination HostRef");
    }
    if profile == Profile::B {
...
async fn main() {
    let result = async {
        ...
        match command.as_str() {
            "host" => {
                let runtime = start_host(paths(required(&args, "app-data-dir")?)?).await?;
                let bridge_id = required(&args, "bridge-id")?;
            ...
            "collect" => {
                ...
                write_report(
                    profile,
                    required(&args, "role")?,
                    &paths(required(&args, "app-data-dir")?)?,
