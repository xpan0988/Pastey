...
function NativeAgentTaskCard({ roomId, peers }: { roomId: string; peers: ReturnType<typeof roomPeers> }) {
  ...
  return <div className="v2-task-open">
    ...
    <footer>{agent.movement?.state === "awaiting_approval" && remote ? <button type="button" className="v2-button primary" disabled={agent.busy} onClick={() => void agent.approveRemoteMovement(roomId, remote.peerSessionId)}>Approve and run</button> : running ? <button type="button" className="v2-button" disabled={agent.busy} onClick={() => void (remote ? agent.cancelRemote(roomId, remote.peerSessionId, remote.hostRef!) : agent.cancel())}>Stop Codex</button> : <button type="button" className="v2-button primary" disabled={agent.busy || !!agent.movement || (!remote && codex?.state !== "available") || (remote && !remote.hostRef) || !agent.workspace.trim() || !agent.taskText.trim()} onClick={() => void (remote ? (workspaceIsLocal ? agent.proposeRemoteMovement(roomId, remote.hostRef!) : agent.startRemote(roomId, remote.peerSessionId, remote.hostRef!)) : agent.start())}>{agent.busy ? "Starting…" : workspaceIsLocal && remote ? "Review movement" : "Run with Codex"}</button>}</footer>
  </div>;
}
