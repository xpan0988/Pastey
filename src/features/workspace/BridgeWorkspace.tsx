...
function NativeAgentTaskCard({ roomId, peers }: { roomId: string; peers: ReturnType<typeof roomPeers> }) {
  const agent = useNativeAgentTask(roomId);
  ...
  return <div className="v2-task-open">
    ...
    {agent.movement ? <section className="v2-task-open"><strong>{agent.movement.state === "awaiting_approval" ? "Review workspace movement" : agent.movement.state.replace(/_/g, " ")}</strong><small>{agent.movement.reviewSummary}</small>{agent.movement.state === "awaiting_approval" ? <ul><li>Send “{agent.movement.sourceWorkspaceName}” to {remote?.displayName ?? "the selected device"}.</li><li>Let Codex work on it.</li><li>Return and apply the result here if the source is unchanged.</li></ul> : null}{remoteReconciliationRequired ? <small className="v2-error">The remote task outcome is indeterminate. Pastey will not reuse this workspace until the task is reconciled or explicitly stopped.</small> : null}{consequenceAbandonmentRequired ? <small className="v2-error">Pastey could not complete result recovery. You can abandon this movement; the Agent’s recorded task outcome will remain unchanged.</small> : null}{resultReturnRetryRequired ? <small className="v2-error">Codex completed the task, but the durable result Return still needs delivery repair.</small> : null}{agent.movement.state === "conflict_recovery_required" ? <><small className="v2-error">Your original workspace changed while Codex was working. Pastey preserved the result and did not overwrite your changes.</small><footer><button type="button" className="v2-button" disabled={agent.busy} onClick={() => void agent.revealConflictResult()}>Reveal result</button><button type="button" className="v2-button" disabled={agent.busy} onClick={() => void agent.discardConflictResult()}>Discard result</button></footer></> : null}</section> : null}
    {agent.message ? <small className="v2-error">{agent.message}</small> : null}
    <footer>{agent.movement?.state === "awaiting_approval" && remote ? <button type="button" className="v2-button primary" disabled={agent.busy} onClick={() => void agent.approveRemoteMovement(roomId, remote.peerSessionId)}>Approve and run</button> : remoteReconciliationRequired ? <><button type="button" className="v2-button primary" disabled={agent.busy || !agent.recoveryTargetHostRef} onClick={() => void agent.reconcileRemote()}>Reconcile</button><button type="button" className="v2-button" disabled={agent.busy} onClick={() => void agent.stopRecovery()}>Stop</button></> : consequenceAbandonmentRequired ? <button type="button" className="v2-button" disabled={agent.busy} onClick={() => void agent.stopRecovery()}>Abandon recovery</button> : resultReturnRetryRequired ? <button type="button" className="v2-button primary" disabled={agent.busy} onClick={() => void agent.retryResultReturn()}>Retry result Return</button> : agent.canCancel ? <button type="button" className="v2-button" disabled={agent.busy} onClick={() => void (remote ? agent.cancelRemote(roomId, remote.peerSessionId, remote.hostRef!) : agent.cancel())}>Stop Codex</button> : <button type="button" className="v2-button primary" disabled={agent.busy || movementBlocksNewRun || (!remote && codex?.state !== "available") || (remote && !remote.hostRef) || !agent.workspace.trim() || !agent.taskText.trim()} onClick={() => void (remote ? (workspaceIsLocal ? agent.proposeRemoteMovement(roomId, remote.hostRef!) : agent.startRemote(roomId, remote.peerSessionId, remote.hostRef!)) : agent.start())}>{agent.busy ? "Starting…" : workspaceIsLocal && remote ? "Review movement" : "Run with Codex"}</button>}</footer>
  </div>;
}
